...
if __name__ == '__main__':
    ...
    chunk_size = 100000
    parts = 1000
    psize = chunk_size//parts


    #h_nonempty_flag = np.zeros(all_nnz, dtype=bool)
    #h_nonempty_flag[h_nonempty_inds] = True
    #m_nonempty_flag = np.isin(umc_subs[:,1], )

    ch = 10**7
    humans_list = umc_subs[:,0]
    #print(humans_list[:100])
    ch_inds = np.where(humans_list < ch)[0]
    #print(ch_inds)
